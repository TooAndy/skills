...
2. 遍历英文信源:
   ...
   4. 获取每条候选:
      ...
      9. 更新 total_count
   10. 检查该信源是否达到最低目标（见下方数据源目标）
3. 遍历中文信源:
   4. 使用 wechat-article-scraper 爬取机器之心公众号各个专栏
   5. 使用 wechat-article-scraper 爬取量子位公众号各个专栏
   6. 提取文章中的产品信息，补充到 index.json
4. 运行充足性检查脚本，确保所有数据源达标
5. 返回 index.json
```

### 数据源最低目标

每个信源必须达到最低目标才能认为该信源搜索充分：

| 信源            | 最低目标 | 说明                              |
| --------------- | -------- | --------------------------------- |
| Hacker News     | 30 条    | AI agent 相关热度帖               |
| GitHub Trending | 20 条    | AI agent 项目                     |
| GitHub Search   | 30 条    | AI agent 仓库                     |
| Reddit          | 10 条    | LocalLLaMA 讨论                   |
| Product Hunt    | 10 条    | AI Agent 产品                     |
| 机器之心        | 20 条    | 微信公众号文章                     |
| 量子位          | 20 条    | 微信公众号文章（当月内容可能较少）  |
...
```bash
./scripts/check_source_sufficiency.sh reports 2026-04
```

**退出条件**:
- ✅ 所有数据源达标 → 进入下一阶段
- ⚠️  中文信源数量有限但已爬完全部文章 → 记录原因后可继续
- ❌ 英文信源未达标 → 必须继续搜索
...
Hacker News: 25 (目标: 30) ❌ 不足
GitHub Trending: 20 (目标: 20) ✅ 充足
量子位: 5 (目标: 20) ⚠️ 数量有限

❌ 仍有数据源需要处理:
  - Hacker News: 25 / 30
  - 量子位: ⚠️ 数量有限，但可能当月内容确实较少

建议:
  - 对于未搜索的信源，请继续搜索
  - 对于数量有限的信源，请确认是否已爬取所有可用文章
```

**中文信源特殊说明**:

对于机器之心、量子位等微信公众号，如果当月确实没有更多文章：
1. 确认已爬取该公众号所有专栏的完整历史
2. 在报告中注明"当月该信源内容较少"
3. 可接受数量有限，继续下一阶段

**注意**: 不能因为总数达到 100 就提前停止，必须确保**每个信源**都搜索充分。
...
1. **机器之心**: 使用 `/wechat-article-scraper` skill 依次爬取上述 6 个专栏，获取文章列表和完整内容
2. **量子位**: 使用 `/wechat-article-scraper` skill 依次爬取上述 2 个专栏，获取文章列表和完整内容
3. **读取已保存的文章内容**: 从 `wechat_articles/{公众号名}_{专栏名}/{文章id}.md` 读取完整原文
4. **提取 AI Agent 相关信息**: 从原文中识别 AI Agent 相关的产品/项目
5. **生成摘要**: 从原文提取 2-3 句摘要
6. **立即 append 到 index.json**

**注意**: 文章完整内容已由 `wechat-article-scraper` 保存到本地，此阶段只需读取和提取摘要。
